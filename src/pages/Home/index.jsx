import "./style.css";

import Centralimage from "../../assets/Sans titre.jpg";
import projet2 from "../../assets/projet2.webp";
import projet3 from "../../assets/projet3.webp";
import projet4 from "../../assets/projet4.webp";
import projet5 from "../../assets/projet5.webp";
import projet6 from "../../assets/projet6.webp";
import projet7 from "../../assets/projet7.webp";
import projet8 from "../../assets/projet8.webp";
import projet9 from "../../assets/projet9.webp";
import projet10 from "../../assets/projet10.webp";
import projet11 from "../../assets/projet11.webp";
import Card from "../../components/Card/index";

function Home() {
  return (
    <main>
      <Card
        cardPosition={"16 / 6 / 25 / 10"}
        cardPositionmedium={" 3 / 1 / 6 / 7"}
        cardPositionsmall={"20 / 1 / 22 / 13"}
        cardSrc={projet2}
        cardUrl={"https://rudidonnot.github.io/Riding-Cities/"}
        cardAlt={
          "Le deuxième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Ce projet nommé Riding Cities fut mes premiers pas en tant que créateur de site web, j'ai du modifier le code de base pour ajouter la section 'Les membres fondateurs' grace à celui ci j'ai appris le premier langage nécessaire à tout développeur web: le HTML. tech.: HTML, CSS."
        }
      />
      <Card
        cardPosition={"27 / 1 / 30 / 6"}
        cardPositionmedium={" 1 / 7 / 4 / 13"}
        cardPositionsmall={"18 / 1 / 20 / 13"}
        cardSrc={projet3}
        cardUrl={"https://rudidonnot.github.io/Booki/"}
        cardAlt={
          "Le troisième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Ce projet nommé Booki m'a permis d'entamer mon apprentissage du CSS et à consolider mon HTML. J'ai appris grace à celui ci tout le sens du terme Responsive. Ainsi, je peux rendre tout site web 'Desktop' cassique compatible aux smartphones. tech.: HTML, CSS."
        }
      />
      <Card
        cardPosition={"16 / 1 / 25 / 6"}
        cardPositionmedium={" 6 / 1 / 9 / 7"}
        cardPositionsmall={"4 / 1 / 6 / 13"}
        cardSrc={projet4}
        cardUrl={" https://rudidonnot.github.io/ohmyfood/"}
        cardAlt={
          "Le quatrième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "C'est le quatrième projet Openclassrooms, il m'a enseigné l'importance d'avoir des éléments interactifs au sein d'un site web. tech.: HTML, CSS."
        }
      />
      <Card
        cardPosition={"14 / 10 / 25 / 13"}
        cardPositionmedium={" 4 / 7 / 7 / 13"}
        cardPositionsmall={"16 / 1 / 18 / 13"}
        cardSrc={projet5}
        cardUrl={"https://rudidonnot.github.io/print-it/"}
        cardAlt={
          "Le cinquième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Print it est un projet consistant à l'appretissage du langage informatique Javascript, j'ai notamment réalisé un Carrousel de A à Z. tech.: HTML, CSS, Javascript."
        }
      />
      <Card
        cardPosition={"3 / 10 / 14 / 13"}
        cardPositionmedium={" 9 / 1 / 14 / 7"}
        cardPositionsmall={"14 / 1 / 16 / 13"}
        cardSrc={projet6}
        cardUrl={"https://rudidonnot.github.io/sophie-bluel/"}
        cardAlt={
          "Le sixième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Grace à ce projet nommé Sophie Bluel j'ai appris à appréhender un Swagger, à respecter les exigences d'un Kanban, mais surtout j'ai consolidé mes bases en Javascript. tech.: HTML, CSS, Javascript."
        }
      />
      <Card
        cardPosition={"27 / 6 / 30 / 10"}
        cardPositionmedium={" 7 / 7 / 10 / 13"}
        cardPositionsmall={"12 / 1 / 14 / 13"}
        cardSrc={projet7}
        cardUrl={
          "https://www.figma.com/design/Q6NEUPqwz1U3HFaCaVoF7N/Maquette-desktop---Menu-Maker-by-Qwenta?node-id=0-6&node-type=frame&t=i2jX9eNxnodCUCur-0"
        }
        cardAlt={
          "Le septième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Ce septième projet nommé Qwenta m'a appris à changer de rôle en entreprise en me mettant à la place d'un Product Owner, j'ai donc rempli les nécessités du post en créant un Kanban, une veille technique, et en organisant les tâches de différents employés à partir d'un mode de plannification Agile: le mode SCRUB."
        }
      />
      <Card
        cardPosition={"27 / 10 / 27 / 13"}
        cardPositionmedium={"16 / 7 / 18 / 13"}
        cardPositionsmall={"10 / 1 / 12 / 13"}
        cardSrc={projet8}
        cardUrl={"https://rudidonnot.github.io/Kasa"}
        cardAlt={
          "Le huitième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Le huitième projet que j'ai effectué s'appelle Kasa, il m'a apprit à créer une application React conforme, tout en appliquant les exigences précédente en terme de Responsive. tech.: HTML, CSS, Javascript, React."
        }
      />
      <Card
        cardPosition={"3 / 1 / 12 / 6"}
        cardPositionmedium={" 10 / 7 / 14 / 13"}
        cardPositionsmall={"8 / 1 / 10 / 13"}
        cardSrc={projet9}
        cardUrl={"https://exemple.com/lien1"}
        cardAlt={
          "Le neuvième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Le neuvième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms."
        }
      />
      <Card
        cardPosition={"7 / 6 / 16 / 10"}
        cardPositionmedium={" 14 / 1 / 18 / 7"}
        cardPositionsmall={"6 / 1 / 8 / 13"}
        cardSrc={projet10}
        cardUrl={"https://exemple.com/lien1"}
        cardAlt={
          "Le dixième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Le dixième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms. tech.: HTML, CSS, Javascript, React, Create React App, React DOM"
        }
      />
      <Card
        cardPosition={"12 / 1 / 16 / 6"}
        cardPositionmedium={" 14 / 7 / 16 / 13"}
        cardPositionsmall={"2 / 1 / 4 / 13"}
        cardSrc={projet11}
        cardUrl={"https://exemple.com/lien1"}
        cardAlt={
          "Le onzième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms"
        }
        cardText={
          "Le onzième projet effectué dans le cadre du diplôme d'intégrateur web d'Openclassrooms. tech.: HTML, CSS, Javascript, React, Create React App, React DOM, Redux"
        }
      />
      <Card
        cardPosition={"3 / 6 / 7 / 10"}
        cardPositionmedium={" 1 / 1 / 3 / 7"}
        cardPositionsmall={"1 / 1 / 2 / 13"}
        cardSrc={Centralimage}
        cardUrl={"http://localhost:3000/"}
        cardAlt={"Photographie de Rudi Donnot"}
        cardText={
          "Je m'appelle Rudi Donnot, 28 ans, en 2023 j'ai entamé une formation d'Intégration Web d'OpenClassrooms, j'ai pu effectuer divers sites webs, notamment ceux présent dans ce portfolio. Chacun de ces sites représente une compétence que j'ai acquise durant ma formation. Cliquez sur chacun d'entre eux pour accéder au travail réalisé !"
        }
      />
    </main>
  );
}

export default Home;
